...
When making own data cable please ensure you are using ribbon cable with first wire (corresponding to first pin in each connector) that has marking, like on pictures above. There is no need to have special splitters. To make one you will need to clamp female IDC connector anywhere on a ribbon. Unfortunately, there is no female Micro-MaTch connectors for wires. Therefore, to make extender you will have to make PCB with one IDC male connector and one or more Micro-MaTch connectors.
